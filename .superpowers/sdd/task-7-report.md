...
- Final retry-safety fix: if all orchestrator updates commit but snapshot refresh fails, Panda returns the committed events with `snapshot_status=unavailable_after_commit` and a warning instead of a retryable failed-update body. Tests: npm test -- server/panda-api.test.mjs; npm test; npm run build. Commit: pending.
