...
- Final no-replay fix: committed-but-no-snapshot responses now return `updates: []`, `no_replay: true`, and the client suppresses local replay for `no_replay` or `snapshot_status=unavailable_after_commit`. Tests: npm test -- server/panda-api.test.mjs; npm test -- src/lib/panda.test.ts; npm test; npm run build. Commit: pending.
