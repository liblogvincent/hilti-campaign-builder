...
const paidMediaLaunch: CampaignArchetype = {
  ...
  steps: [
    ...
    { id: "utm", kind: "agent", label: "UTM Generation", depends_on: ["localization"], task_type: "utm_create_qa", cardinality: "exactly_one" },
    { id: "rollout", kind: "tool", label: "Roll-out", depends_on: ["utm"], task_type: "rollout_sequence", cardinality: "exactly_one" },
    { id: "qa_structural", kind: "agent", label: "Structural QA", depends_on: ["rollout"], task_type: "voice_fit_review", cardinality: "exactly_one" },
    { id: "h3", kind: "gate", label: "H3 — Publish Gate", gate: "H3", depends_on: ["qa_structural"], task_type: "publish_approval", cardinality: "exactly_one" },
