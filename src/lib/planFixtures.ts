...
export const FIXTURE_PLAN: AdaptedPlan = {
  ...
  nodes: [
    ...
    { id: "brief", label: "Brief Intake", kind: "agent", status: "done", depends_on: [], task_id: "brief_intake" },
    { id: "strategy", label: "Strategy Plan", kind: "agent", status: "done", depends_on: ["brief"], task_id: "campaign_plan" },
    { id: "h1", label: "H1 — Plan Approval", kind: "gate", gate: "H1", status: "blocked", depends_on: ["strategy"], decision: null },
    // Phase 2 — Content (after H1)
    { id: "content", label: "Content Gen", kind: "agent", status: "waiting", depends_on: ["h1"], task_id: "create_ad_set" },
    { id: "qa", label: "QA Check", kind: "agent", status: "waiting", depends_on: ["content"], task_id: "voice_fit_review" },
    ...
    { id: "localization", label: "Localization", kind: "agent", status: "waiting", depends_on: ["h2"], task_id: "localize" },
    { id: "utm", label: "UTM Generation", kind: "agent", status: "waiting", depends_on: ["localization"], task_id: "utm_create_qa" },
    { id: "rollout", label: "Roll-out", kind: "tool", status: "waiting", depends_on: ["utm"] },
    { id: "qa_structural", label: "Structural QA", kind: "agent", status: "waiting", depends_on: ["rollout"], task_id: "voice_fit_review" },
    ...
    { id: "learn", label: "Learn & Improve", kind: "agent", status: "waiting", depends_on: ["h3"], task_id: "insights" },
